...
function trx_utils_registration_validate(form) {
	...
	if (!error) {
		jQuery.post(TRX_UTILS_STORAGE['ajax_url'], {
			...
			nonce: TRX_UTILS_STORAGE['ajax_nonce'],
			user_name: 	form.find('[id^="registration_username_"]').val(),
			user_email: form.find('[id^="registration_email_"]').val(),
			...
			if (rez.error === '') {
				...
				result_box.addClass('sc_infobox sc_infobox_style_error').html(TRX_UTILS_STORAGE['msg_registration_error'] + ' ' + rez.error);
			}
			result_box.fadeIn().delay(3000).fadeOut();
		});
	}
